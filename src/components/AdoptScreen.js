...
const AdoptScreen = ({ currentUser, onBack, onStartChat }) => {
  ...
  const [petOtherBreed, setPetOtherBreed] = useState('');
  ...
  const handleAddAdoptablePet = () => {
    if (
      ...
      !petDescription ||
      !petPhotoPreview ||
      (petType === 'Otro' && !petOtherType) ||
      !petBreed ||
      (petBreed === 'Otro' && !petOtherBreed)
    ) {
    ...
    const newPet = {
      ...
      breed: petBreed === 'Otro' ? petOtherBreed : petBreed,
          ...
          {(petType === "Perro" || petType === "Gato") && (
            <>
              ...
              >
                ...
                {(petType === "Perro" ? dogBreeds : catBreeds).map((breed) => (
              ...
              {petBreed === "Otro" && (
                <input
                  ...
                  value={petOtherBreed}
                  onChange={(e) => setPetOtherBreed(e.target.value)}
                  className="w-full px-4 py-2 mb-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                />
              )}
            </>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
        ...
        {adoptablePets.length === 0 ? (
          <p className="text-center text-gray-500">
            No hay mascotas disponibles para adopción con estos filtros.
          </p>
        ) : (
          adoptablePets.map((pet) => (
            ...
            >
              ...
              <p className="text-gray-500 text-xs mt-2">
                Publicado por: {pet.rescuerName} ({pet.vetName})
              </p>

              {currentUser.type === "rescatista" &&
                currentUser.id === pet.rescuerId && (
                  <button
              ...
              {currentUser.type === "adoptante" && (
                <button
                  onClick={() =>
                    onStartChat(
                      pet.rescuerId,
                      `¡Me interesa la mascota ${pet.name}! ¿Podemos hablar sobre la adopción?`
                    )
                  }
