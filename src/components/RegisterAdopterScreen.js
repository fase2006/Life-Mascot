...
const RegisterAdopterScreen = ({ onRegisterSuccess, onBackToWelcome }) => {
  ...
  const handleRegister = (e) => {
    e.preventDefault();
    setError('');
    if (!name || !email || !emailDomain || !password) {
    ...
    addUser({ id: Date.now(), name, email: fullEmail, password, type: 'adoptante', nickname: `@${name.toLowerCase().replace(/\s/g, '')}`, profilePhoto: '', description: '', registeredDate: new Date().toLocaleDateString(), isPremium: false });
    onRegisterSuccess();
  };
  ...
  return (
    ...
    >
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 relative z-10">
        <LogoHeader />
        <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Regístrate como Adoptante</h2>
...
export default RegisterAdopterScreen;